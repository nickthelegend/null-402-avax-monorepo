...
export interface VerifyResult {
   ...
   *  On-chain path: the EVM call/tx ref. Dev path: a local tag. */
  proofRef: string;
  /** "evm" = real on-chain Groth16 (Avalanche) · "local" = real Groth16 verified
   *  off-chain with snarkjs · "dev" = insecure local scaffold. */
  mode: "evm" | "local" | "dev";
}
...
/** Avalanche/EVM configuration shared by client and server. */
